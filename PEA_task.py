    ...
    if df is not None:
        ...
        if emp_id:
            ...
            if df_emp.empty:
                ...
                job_count_df['หมายเลข'] = job_count_df['รายการงานที่ทำ'].apply(lambda x: int(x.split('.')[0]) if x.split('.')[0].isdigit() else 0)
                ...
                st.markdown(f"### พนักงานรหัส {emp_id} ทำงานรวมทั้งหมด **{total_jobs}** ครั้งในเดือนนี้")
